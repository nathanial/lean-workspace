  ...
  collect := fun layout =>
    ...
    do
      ...
      for i in [0:barCount] do
        ...
        CanvasM.fillRectColor' barX barY barWidth barHeight color dims.cornerRadius
...
def barChart (data : Dyn (Array Float)) (labels : Array String := #[])
  ...
  let _ ← dynWidget data fun currentData => do
    ...
    emitM do pure (barChartVisual name currentData labels variant theme dims)
...
def multiColorBarChart (data : Dyn (Array BarChart.DataPoint))
  ...
  let _ ← dynWidget data fun currentData => do
    let name ← registerComponentW (isInteractive := false)
    emitM do pure (multiColorBarChartVisual name currentData theme dims)
