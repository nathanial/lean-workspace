  ...
  collect := fun layout =>
    ...
    do
      ...
      if showPlaceholder then
        ...
        CanvasM.fillTextId placeholder rect.x textY theme.font theme.textMuted
      else
        ...
        for i in [:lines.size] do
          ...
          | some line =>
            ...
            if lineY + lineHeight >= rect.y && lineY < rect.y + viewportH then
              ...
              CanvasM.fillTextId line.text rect.x textY theme.font theme.text
          | none => pure ()
...
def textArea (placeholder : String) (initialState : TextAreaState)
  ...
  let _ ← dynWidget renderState fun (state, focused) => do
    ...
    emitM do pure (textAreaVisual name theme { state with focused := isFoc } placeholder width height)
