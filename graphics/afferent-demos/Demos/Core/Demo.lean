...
structure RunnerStats where
  ...
  layoutTrackedMs : Float := 0.0
  layoutStrictValidationChecks : Nat := 0
  layoutStrictValidationFailures : Nat := 0
  measureCacheHits : Nat := 0
  measureCacheMisses : Nat := 0
  measureCacheBypasses : Nat := 0
  measureCacheLookupMs : Float := 0.0
  measureCacheComputeMs : Float := 0.0
