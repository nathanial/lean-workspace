...
private def spinningStarsCommands (r : Rect) (t : Float) : CanvasM Unit := do
  ...
  for i in [:5] do
    ...
    CanvasM.pushTranslate x y
    CanvasM.pushRotate (t * 3.0 + i.toFloat)
    CanvasM.fillPathColor (Afferent.Path.star ⟨0, 0⟩ baseRadius (baseRadius * 0.5) 5) (Afferent.Color.hsv hue 1.0 1.0)
    CanvasM.popTransform
    CanvasM.popTransform
  CanvasM.popTransform

/-- Pulsing rainbow circles. -/
private def pulsingCirclesCommands (r : Rect) (t : Float) : CanvasM Unit := do
  let center := rectCenter r
  ...
  for i in [:10] do
    ...
    CanvasM.fillPathColor (Afferent.Path.circle ⟨x, y⟩ radius) (Afferent.Color.hsv hue 1.0 1.0)

/-- Wiggling line path. -/
private def wigglingLineCommands (r : Rect) (t : Float) : CanvasM Unit := do
  ...
  CanvasM.strokePathColor path (Afferent.Color.hsv hue 1.0 1.0) 3.0

/-- Morphing polygon. -/
private def morphingPolygonCommands (r : Rect) (t : Float) : CanvasM Unit := do
  let center := rectCenter r
  let sides := 3 + ((t * 0.5).floor.toUInt32 % 6).toNat
  let radius := minSide r * 0.35 + minSide r * 0.08 * Float.sin t
  let hue := (t * 0.4) - (t * 0.4).floor
  CanvasM.pushTranslate center.x center.y
  CanvasM.pushRotate (t * 1.5)
  CanvasM.fillPathColor (Afferent.Path.polygon ⟨0, 0⟩ radius sides) (Afferent.Color.hsv hue 0.8 0.9)
  CanvasM.popTransform
  CanvasM.popTransform

/-- Orbiting hearts with trail effect. -/
private def orbitingHeartsCommands (r : Rect) (t : Float) : CanvasM Unit := do
  let center := rectCenter r
  ...
  for i in [:6] do
    ...
    CanvasM.pushTranslate x y
    CanvasM.pushScale (0.25 + 0.1 * Float.sin (t * 3.0)) (0.25 + 0.1 * Float.sin (t * 3.0))
    CanvasM.fillPathColor (Afferent.Path.heart ⟨0, 0⟩ (minSide r * 0.6)) (Afferent.Color.rgba color.r color.g color.b alpha)
    CanvasM.popTransform
    CanvasM.popTransform

/-- Bouncing rectangles. -/
private def bouncingRectsCommands (r : Rect) (t : Float) : CanvasM Unit := do
  ...
  for i in [:5] do
    ...
    let hue := (t * 0.5 + i.toFloat / 5.0) - (t * 0.5 + i.toFloat / 5.0).floor
    CanvasM.pushTranslate x y
    CanvasM.pushRotate (t * 2.0 + phase)
    CanvasM.fillRectColor (Rect.mk' (-size / 2) (-size / 2) size size) (Afferent.Color.hsv hue 0.9 1.0)
    CanvasM.popTransform
    CanvasM.popTransform
