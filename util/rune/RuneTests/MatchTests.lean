...
-- ============================================================================
-- matchAt / matchPrefix Tests
-- ============================================================================

test "matchAt basic" := do
  let re ← compile! "[a-z]+"
  -- Match at position 0
  if let some m := re.matchAt "hello123" 0 then
    m.text ≡ "hello"
    m.start ≡ 0
    m.stop ≡ 5
  else
    panic! "expected match at position 0"
  -- No match at position 0 when digits come first
  shouldSatisfy (re.matchAt "123abc" 0).isNone "should not match digits at position 0"
  -- Match at position 3 (where letters start)
  if let some m := re.matchAt "123abc" 3 then
    m.text ≡ "abc"
    m.start ≡ 3
    m.stop ≡ 6
  else
    panic! "expected match at position 3"

test "matchAt does not search" := do
  let re ← compile! "[a-z]+"
  -- Unlike find, matchAt does not search for a match
  -- If no match starts at the given position, returns none
  shouldSatisfy (re.matchAt "123abc" 0).isNone "should not search past position 0"
  shouldSatisfy (re.matchAt "123abc" 1).isNone "should not search past position 1"
  shouldSatisfy (re.matchAt "123abc" 2).isNone "should not search past position 2"
  shouldSatisfy (re.matchAt "123abc" 3).isSome "should match at position 3"

test "matchPrefix basic" := do
  let re ← compile! "[a-z]+"
  -- Match at start
  if let some m := re.matchPrefix "hello123" then
    m.text ≡ "hello"
    m.start ≡ 0
  else
    panic! "expected match at start"
  -- No match at start
  shouldSatisfy (re.matchPrefix "123hello").isNone "should not match when digits first"

test "matchPrefix vs isMatch" := do
  let re ← compile! "[a-z]+"
  -- matchPrefix matches a prefix, doesn't require consuming entire string
  shouldSatisfy (re.matchPrefix "hello123").isSome "matchPrefix accepts trailing content"
  -- isMatch requires entire string to match
  shouldSatisfy (re.isMatch "hello123").isNone "isMatch rejects trailing content"
  shouldSatisfy (re.isMatch "hello").isSome "isMatch accepts full match"

test "matchPrefix with captures" := do
  let re ← compile! "([a-z]+)([0-9]+)"
  if let some m := re.matchPrefix "abc123xyz" then
    m.text ≡ "abc123"
    m.group 1 ≡ some "abc"
    m.group 2 ≡ some "123"
  else
    panic! "expected match with captures"

test "matchAt with captures" := do
  let re ← compile! "([a-z]+)@([a-z]+)"
  if let some m := re.matchAt "xxx user@domain yyy" 4 then
    m.text ≡ "user@domain"
    m.group 1 ≡ some "user"
    m.group 2 ≡ some "domain"
    m.start ≡ 4
    ...
    panic! "expected match at position 4"

test "matchAt at end of string" := do
  let re ← compile! "[a-z]+"
  shouldSatisfy (re.matchAt "hello" 5).isNone "no match at end position"
  shouldSatisfy (re.matchAt "hello" 10).isNone "no match past end position"

test "matchPrefix empty pattern" := do
  let re ← compile! "a*"
  -- Empty pattern matches at start with empty string
  if let some m := re.matchPrefix "123" then
    m.text ≡ ""  -- Zero 'a's matched
    m.start ≡ 0
    m.stop ≡ 0
  else
    panic! "expected empty match"
