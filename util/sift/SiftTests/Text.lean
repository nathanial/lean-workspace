...
test "jsonNumber parses integer" := do
  match Parser.run jsonNumber "42" with
  | .ok f => if (f - 42.0).abs < 0.001 then pure () else panic! s!"expected 42, got {f}"
  | .error e => panic! s!"expected success: {e}"

test "jsonNumber parses zero" := do
  match Parser.run jsonNumber "0" with
  | .ok f => if f.abs < 0.001 then pure () else panic! s!"expected 0, got {f}"
  | .error e => panic! s!"expected success: {e}"

test "jsonNumber parses negative integer" := do
  match Parser.run jsonNumber "-42" with
  | .ok f => if (f - (-42.0)).abs < 0.001 then pure () else panic! s!"expected -42, got {f}"
  | .error e => panic! s!"expected success: {e}"

test "jsonNumber parses decimal" := do
  match Parser.run jsonNumber "3.14" with
  | .ok f => if (f - 3.14).abs < 0.001 then pure () else panic! s!"expected 3.14, got {f}"
  | .error e => panic! s!"expected success: {e}"

test "jsonNumber parses zero decimal" := do
  match Parser.run jsonNumber "0.5" with
  | .ok f => if (f - 0.5).abs < 0.001 then pure () else panic! s!"expected 0.5, got {f}"
  | .error e => panic! s!"expected success: {e}"

test "jsonNumber parses scientific notation" := do
  match Parser.run jsonNumber "1.5e10" with
  | .ok f => if (f - 1.5e10).abs < 1.0 then pure () else panic! s!"expected 1.5e10, got {f}"
  | .error e => panic! s!"expected success: {e}"

test "jsonNumber parses negative exponent" := do
  match Parser.run jsonNumber "2.5e-3" with
  | .ok f => if (f - 0.0025).abs < 0.0001 then pure () else panic! s!"expected 0.0025, got {f}"
  | .error e => panic! s!"expected success: {e}"

test "jsonNumber parses integer with exponent" := do
  match Parser.run jsonNumber "5e3" with
  | .ok f => if (f - 5000.0).abs < 0.1 then pure () else panic! s!"expected 5000, got {f}"
  | .error e => panic! s!"expected success: {e}"

test "jsonNumber parses uppercase E exponent" := do
  match Parser.run jsonNumber "1E6" with
  | .ok f => if (f - 1000000.0).abs < 0.1 then pure () else panic! s!"expected 1000000, got {f}"
  | .error e => panic! s!"expected success: {e}"

test "jsonNumber parses positive exponent sign" := do
  match Parser.run jsonNumber "1e+3" with
  | .ok f => if (f - 1000.0).abs < 0.1 then pure () else panic! s!"expected 1000, got {f}"
  | .error e => panic! s!"expected success: {e}"

test "jsonNumber rejects leading plus sign" := do
  match Parser.run jsonNumber "+42" with
  | .error _ => pure ()  -- correct: + is not consumed, parsing starts at digit
  | .ok _ => panic! "expected failure - JSON numbers don't allow leading +"

test "jsonNumber rejects leading zeros" := do
  match Parser.run jsonNumber "007" with
  | .error _ => pure ()
  | .ok _ => panic! "expected failure - JSON numbers don't allow leading zeros"

test "jsonNumber rejects leading zero before integer" := do
  match Parser.run jsonNumber "01" with
  | .error _ => pure ()
  | .ok _ => panic! "expected failure - JSON numbers don't allow leading zeros"

test "jsonNumber parses complex scientific notation" := do
  match Parser.run jsonNumber "-123.456e-7" with
  | .ok f => if (f - (-123.456e-7)).abs < 1e-12 then pure () else panic! s!"expected -123.456e-7, got {f}"
  | .error e => panic! s!"expected success: {e}"

test "jsonNumber parses zero with exponent" := do
  match Parser.run jsonNumber "0e5" with
  | .ok f => if f.abs < 0.001 then pure () else panic! s!"expected 0, got {f}"
  | .error e => panic! s!"expected success: {e}"

test "jsonNumber parses zero with fraction and exponent" := do
  match Parser.run jsonNumber "0.0e10" with
  | .ok f => if f.abs < 0.001 then pure () else panic! s!"expected 0, got {f}"
  | .error e => panic! s!"expected success: {e}"
