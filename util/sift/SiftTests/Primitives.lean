...
test "string fails on empty input" := do
  match Parser.run (string "hello") "" with
  | .error _ => pure ()
  | .ok _ => panic! "expected failure"

test "eof succeeds on empty input" := do
  match Parser.run eof "" with
  | .ok () => pure ()
  | .error _ => panic! "expected success"

test "take 0 succeeds on empty input" := do
  match Parser.run (take 0) "" with
  | .ok s => s ≡ ""
  | .error _ => panic! "expected success"

test "takeWhile returns empty on empty input" := do
  match Parser.run (takeWhile Char.isDigit) "" with
  | .ok s => s ≡ ""
  | .error _ => panic! "expected success"
