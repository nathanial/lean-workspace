...
structure ParseState (σ : Type) where
  ...
  pos : Nat
  /-- Start of parsing window (for substring parsing) -/
  startOffset : Nat := 0
  /-- End of parsing window (for substring parsing) -/
  endOffset : Nat
...
def init {σ : Type} (input : String) (userState : σ) : ParseState σ :=
  { input, pos := 0, startOffset := 0, endOffset := input.utf8ByteSize, line := 1, column := 1, userState }

/-- Create initial state from a substring and user state -/
def initSubstring {σ : Type} (sub : Substring.Raw) (userState : σ) : ParseState σ :=
  { input := sub.str
  , pos := sub.startPos.byteIdx
  , startOffset := sub.startPos.byteIdx
  , endOffset := sub.stopPos.byteIdx
  , line := 1
  , column := 1
  , userState }

/-- Check if at end of parsing window -/
...
/-- Run a parser on a substring (for parsers with no user state) -/
def runSubstring {α : Type} (p : Parser Unit α) (input : Substring.Raw) : Except ParseError α :=
  match p (ParseState.initSubstring input ()) with
  | .ok (a, _) => .ok a
  | .error e => .error e

/-- Run a parser on a substring with initial user state -/
def runSubstringWith {σ α : Type} (p : Parser σ α) (input : Substring.Raw) (initState : σ) : Except ParseError α :=
  match p (ParseState.initSubstring input initState) with
  | .ok (a, _) => .ok a
  | .error e => .error e

/-- Run a parser on a substring, requiring it to consume all input in the substring -/
def parseSubstring {α : Type} (p : Parser Unit α) (input : Substring.Raw) : Except ParseError α :=
  match p (ParseState.initSubstring input ()) with
  | .ok (a, finalState) =>
    if finalState.atEnd then .ok a
    else .error (ParseError.fromState finalState "unexpected trailing input")
...
/-- Run a parser on a substring with user state, requiring it to consume all input in the substring -/
def parseSubstringWith {σ α : Type} (p : Parser σ α) (input : Substring.Raw) (initState : σ) : Except ParseError α :=
  match p (ParseState.initSubstring input initState) with
  | .ok (a, finalState) =>
    if finalState.atEnd then .ok a
    else .error (ParseError.fromState finalState "unexpected trailing input")
