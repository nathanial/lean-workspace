...
/-- Parse a JSON-compliant number (RFC 8259).
    Rules:
    - No leading `+` sign (only `-` allowed)
    - No leading zeros in integer part (except `0` alone or `0.xxx`)
    - Optional fractional part: `.` followed by one or more digits
    - Optional exponent part: `e`/`E`, optional sign, one or more digits -/
def jsonNumber {σ : Type} : Parser σ Float := do
  -- Optional negative sign (no + allowed)
  let neg ← optional (char '-')
  -- Integer part: either "0" or "1-9" followed by digits
  let intDigits ← do
    let first ← digit
    if first == '0' then
      -- After leading 0, next char must not be a digit
      match ← peek with
      | some c =>
        if c.isDigit then
          Parser.fail "leading zeros not allowed in JSON numbers"
        else
          pure #[first]
      | none => pure #[first]
    else
      -- First digit is 1-9, collect remaining digits
      let rest ← many digit
      pure (#[first] ++ rest)
  let intPart := intDigits.foldl (fun acc c => acc * 10 + (c.toNat - '0'.toNat)) 0
  let mut result := intPart.toFloat
